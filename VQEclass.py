...
class hamiltonian():
    ...
    def cost_function_VQE(self, theta: list, h: float) -> float:
        ...
        circuit = hardware_efficient_ansatz('0000', theta, self.number_qubits, 2, self.number_ansatz_repetition)
        ...
        circuit = hardware_efficient_ansatz('0000', theta, self.number_qubits, 1, self.number_ansatz_repetition)
    ...
    def cost_function_VQT(self, theta: list) -> float:
        func_val = 0.0

        prob = theta[:self.number_qubits]
        prob_circuit = prob_function_ansatz(prob, self.number_qubits, self.number_qubits)
        job = execute( prob_circuit, self.backend, shots=self.shots)
        result_prob = job.result().get_counts()

        for state in result_prob:
            local_probability = result_prob[state]/self.shots
            func_val -= local_probability*np.log(local_probability)

        for state in result_prob:
            density = hardware_efficient_ansatz(state, theta[self.number_qubits:], self.number_qubits, 2, self.number_ansatz_repetition)
            local_probability = result_prob[state]/self.shots
            for i, hamiltonian_term in enumerate(self.hamiltonian_list_index):
                circuit_copy = density
                for j, component in enumerate(hamiltonian_term):
                    if 'X' == component[0]:
                        circuit_copy.h( component[1] )
