...
'''
class variational_quantum_thermalizer():
    dev = None
    node = None
    backend = None
    optimization_method = ""
    ...
    def __init__(self, params_alg) -> None:
    ...
    def thermalizer_state_calculation(self, theta: list, temperature: float ):
        general_cost_function = lambda theta: self.cost_function_VQT(theta, np.divide(1.0, temperature))
    ...
    def cost_function_VQT(self, theta: list, beta) -> float:
        ...
        for i, term in enumerate(self.hamiltonian_index):
            result_term = self.node( qubits= self.number_qubits, spin=self.spin, rotation_params = rotation, coupling_params = coupling, wire = term)
...
class variational_quantum_eigensolver(Given_ansatz):
    dev = None
    node = None
    backend = None
    optimization_method = ""
    ...
    def __init__(self, params_alg) -> None:
        self.dev = qml.device(params_alg['backend'], wires=self.number_qubits, shots=self.shots)
        self.node = qml.QNode(self.spin_circuit, self.dev, interface="autograd")
        self.optimization_alg_params = params_alg['optimization_alg_params'],
        self.optimization_method = params_alg['optimization_method'],
        pass
